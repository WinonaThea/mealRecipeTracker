...
def create_recipe() : 
    ...
    1. Menambahkan resep baru ke koleksi pribadi pengguna
    2. Membagikan resep pribadi ke koleksi publik
    3. Kembali ke menu utama

    Pengguna dapat : 
    - Memasukkan detail lengkap resep seperti nama, durasi masak, rating, 
    apakah resep termasuk favorit, tingkat kesulitan, instruksi, dan kategori makanan
    - Menambahkan daftar bahan masakan dengan rincian jumlah, satuan, kalori per unit, dan biaya per unit
    - Validasi input agar sesuai dengan format dan tipe data yang diinginkan
    - Memilih apakah ingin menambah resep baru ke koleksi pribadi atau membagikan resep ke koleksi umum
    - Memberikan konfirmasi terakhir sebelum menambah atau membagikan resep
    
    Fungsi ini akan terus berjalan dalam loop hingga pengguna memilih untuk kembali ke menu utama
    """
        ...
        while True: 
            ...
            if input_menu_create.isdigit(): 
                ...
                if input_menu_create == 1: 
                    ...
                    while True : 
                        time_min = input('lama pembuatan (menit), tidak boleh pakai koma atau titik: ')
                        if time_min.isdigit(): 
                    ...
                    while True:
                        ...
                        if yakin.isdigit():
                            ...
                            if yakin == 1 : 
                                ...
                                resep_lain.append(resep_ku[(id_share-1)].copy())
...
def input_kategori_untuk_browse_recipes(resep_ku) : 
    """
    Fungsi ini digunakan untuk menampilkan daftar resep berdasarkan kategori yang dipilih oleh pengguna.
    Pengguna dapat memilih salah satu dari 5 kategori resep: 
    1. main : makanan utama
    2. dessert : makanan penutup
    3. snack : makanan ringan
    4. drink : minuman
    5. side : lauk pendamping

    Setelah memilih kategori, fungsi akan : 
    - Memfilter resep-resep sesuai kategori yang dipilih
    - Menampilkan seluruh informasi resep termasuk bahan masakan (nama, jumlah, satuan, kalori per unit, dan harga per unit),
    serta informasi lainnya seperti instruksi, waktu pembuatan, rating, apakah favorit, tingkat kesulitan memasak, dan status share
    - Jika pada kategori yang dipilih tidak ada resep dalam database, maka akan menampilkan pesan bahwa tidak ada data

    Terdapat validasi input untuk memastikan hanya angka 1 sampai 5 yang diterima
    """
...
def input_tingkat_kesulitan_browse_recipe(resep_ku) : 
    """
    Fungsi ini menampilkan daftar resep berdasarkan tingkat kesulitan memasak yang dipilih oleh pengguna
    Ada 3 tingkat kesulitan: 
    1. mudah
    2. menengah
    3. sulit

    setelah pengguna memilih, fungsi akan:
    - menyaring daftar resep berdasarkan pilihan tingkat kesulitan pengguna
    - menampilkan seluruh informasi resep

    jika tidak ditemukan resep pada tingkat kesulitan yang dipilih, maka akan 
    ditampilkan pesan tidak ada menu di tingkat kesulitan tersebut.
    
    Input juga ada validasi untuk menerima hanya angka 1-3.
    """
...
def fav(resep_ku) : 
    """
    Fungsi ini untuk menampilkan seluruh resep yang ditandai sebagai favorit oleh pengguna.
    Fungsi ini menyaring daftar resep makanan yang nilai kolom favoritnya bernilai 'True'.
    Ia akan menampilkan seluruh informasi resep favorit tersebut.

    Jika tidak ditemukan resep yang termasuk dalam kategori favorit, maka akan ditampilkan pesan
...
def total_biaya_per_resep(resep_ku):
    """
    Fungsi ini menghitung dan menampilkan total biaya dari setiap resep berdasarkan jumlah dan harga per unit bahan-bahannya

    Fungsi ini akan mengalikan jumlah bahan dengan harga per unit untuk setiap bahan dalam resep
    dan kemudian menjumlahkan total biayanya dalam satu resep. 

    Hasil perhitungannya akan ditampilkan dalam bentuk tabel 
...
def total_kalori_per_resep(resep_ku):
    """
    Fungsi ini menghitung dan menampilkan total kalori dari setiap resep 
    berdasarkan jumlah dan kalori per unit dari bahan-bahannya.

    Fungsi ini akan mengalikan jumlah bahan dengan kalori per unit untuk setiap
    bahan dalam resep, dan kemudian menjumlahkan total kalori dalam resep.
    ...
    print(f'{"id resep":<10} | {"nama":<20} | {"total kalori":<10}')
    print('-' * 50)
...
def lihat_semua_resep(resep_ku):
    """
    Fungsi ini menampilkan seluruh resep dalam koleksi.
    Fungsi ini akan menampilkan setiap baris resep secara terpisah untuk 
    tiap bahan yang ada di dalamnya
    Fungsi ini akan menampilkan hasil dalam bentuk tabel, meliputi : 
    - id resep
    - nama makanan
    - kategori makanan
    - nama bahan
...
def browse_recipes() : 
    """
    Fungsi ini digunakan untuk : 
    1. Browsing resep dari koleksi pribadi
    2. Browsing resep dari koleksi umum
    3. Kembali ke menu awal

    Di masing-masing koleksi, pengguna dapat:
    - Melihat semua resep
    - Melihat resep berdasarkan : 
        - Kategori makanan (main/ dessert/ snack/ drink/ side)
        - Tingkat kesulitan memasak (mudah/ menengah/ sulit)
        - Resep Favorit
        - Total biaya per resep
        - Total kalori per resep
    
    Fungsi mempunyai validasi untuk memastikan input sesuai dengan opsi yang tersedia.
    Fungsi akan terus berjalan hingga pengguna memilih untuk kembali ke menu awal.
...
def update_recipes():
    """
    Fungsi ini digunakan untuk : 
    1. update resep dari koleksi pribadi
    2. update status favorit dari resep yang ada pada koleksi umum

    Untuk koleksi pribadi, fungsi akan: 
    - Menampilkan semua resep untuk pengguna pilih resep mana yang akan diupdate, dengan meminta input id resep
    - jika resep sudah pernah dibagikan ('is_shared' == True), maka resep versi publiknya akan dihapus 
    - Pengguna dapat memilih bagian yang ingin diupdate, termasuk: 
        - nama
        - kategori
        - bahan
        - instruksi
    ...
    Untuk koleksi umum, fungsi akan: 
    - Menampilkan semua resep publik
    - Meminta input id resep yang ingin diperbarui (hanya bagian favoritnya saja)
    dengan pengguna dapat memilih status favorit menjadi 'True' atau 'False'

    Fungsi juga punya validasi untuk semua input dan meminta konfirmasi pengguna
    sebelum resep mau diupdate
...
def delete_recipes() : 
    """
    Fungsi akan menghapus resep dari koleksi pribadi.
    1. Fungsi awalnya akan menampilkan semua resep dari koleksi pribadi
    2. Meminta input Id resep yang ingin dihapus
    3. Meminta konfirmasi pengguna sebelum menghapus
    4. Jika resep yang dihapus juga telah dibagikan ke publik, maka akan dihapus juga dari koleksi publik
    5. Setelah dihapus, fungsi akan mengurutkan kembali id resep di kedua koleksi pribadi dan umum agar tetap berurutan

    Fungsi akan menampilkan dua pilihan: 
    - Hapus Resep
    - Kembali ke menu utama

    Fungsi juga punya validasi input 
...
def main_menu():
    """
    Fungsi ini merupakan titik masuk utama aplikasi dan memiliki dua jalur utama:
    1. Pengguna dengan akun
        - Dapat login menggunakan username dan password
        - Jika login berhasil pengguna diberikan akses ke menu utama : 
            1. Create Recipe
            2. Browse Recipe
            3. Update Recipe
            4. Delete Recipe
            5. Exit Program
        - Fungsi juga punya validasi input dilakukan di tiap tahap
    2. Pengguna tanpa akun
        - Ditawari melakukan registrasi (membuat username dan password)
        - Jika menolak registrasi, pengguna hanya dapat melakukan browsing resep koleksi publik
        - Resep ditampilkan dalam format tabel, tanpa kemampuan menambah/mengubah/menghapus resep.

    Fungsi akan terus berjalan hingga pengguna memilih keluar dari program atau hanya browsing sebagai pengguna tanpa akun.
    """
